...
<x-admin-layout>
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        ...
        <!-- Welcome Section -->
        <div class="mb-8">
            <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-slate-200 tracking-tight break-words">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <!-- Total Users -->
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 sm:p-6 transition-all hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                    <div class="min-w-0 w-full break-words">
                        <p class="text-xs sm:text-sm font-medium text-gray-500 dark:text-slate-400">Total Users</p>
                        <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-gray-900 dark:text-slate-200">{{ $stats['total_users'] }}</p>
                    ...
                    <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-primary-50 dark:bg-primary-900/30 rounded-xl flex items-center justify-center">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 text-primary-600 dark:text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 sm:p-6 transition-all hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                    <div class="min-w-0 w-full break-words">
                        <p class="text-xs sm:text-sm font-medium text-gray-500 dark:text-slate-400">Total Roles</p>
                        <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-gray-900 dark:text-slate-200">{{ $stats['total_roles'] }}</p>
                        <p class="mt-1 text-xs text-amber-600 dark:text-amber-400 truncate">
                            Role-based access control
                        </p>
                    </div>
                    <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-amber-50 dark:bg-amber-900/30 rounded-xl flex items-center justify-center">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 sm:p-6 transition-all hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                    <div class="min-w-0 w-full break-words">
                        <p class="text-xs sm:text-sm font-medium text-gray-500 dark:text-slate-400">Total Menus</p>
                        <p class="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-gray-900 dark:text-slate-200">{{ $stats['total_menus'] }}</p>
                        <p class="mt-1 text-xs text-cyan-600 dark:text-cyan-400 truncate">
                            Sidebar navigation items
                        </p>
                    </div>
                    <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-cyan-50 dark:bg-cyan-900/30 rounded-xl flex items-center justify-center">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 text-cyan-600 dark:text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7"></path></svg>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 sm:p-6 transition-all hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                    <div class="min-w-0 w-full break-words">
                        <p class="text-xs sm:text-sm font-medium text-gray-500 dark:text-slate-400">Last Login</p>
                        <p class="mt-1 sm:mt-2 text-base sm:text-lg font-bold text-gray-900 dark:text-slate-200 truncate">
                            {{ auth()->user()->last_login_at ? auth()->user()->last_login_at->diffForHumans() : 'N/A' }}
                        </p>
                        <p class="mt-1 text-xs text-gray-500 dark:text-slate-400 truncate w-full" title="{{ auth()->user()->email }}">
                            {{ auth()->user()->email }}
                        </p>
                    </div>
                    <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl flex items-center justify-center">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                </div>
            </div>
        </div>
