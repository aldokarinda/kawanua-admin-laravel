<x-admin-layout>
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        <x-breadcrumb :items="[['label' => 'Master Data', 'url' => '#'], ['label' => 'Tags', 'url' => route('admin.tags.index')], ['label' => 'Edit', 'url' => '#']]" />

        <div class="mt-4">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Tag: {{ $tag->name }}</h1>
        </div>

        <div class="mt-6 max-w-3xl glass-panel rounded-xl shadow-sm border border-slate-200 dark:border-slate-700/50 p-6">
            <form action="{{ route('admin.tags.update', $tag->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <div>
                        <x-input-label for="name" value="Tag Name" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $tag->name)" required autofocus />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="slug" value="Slug (URL)" />
                        <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full" :value="old('slug', $tag->slug)" required />
                        <x-input-error :messages="$errors->get('slug')" class="mt-2" />
                    </div>
                </div>

                <div class="mt-6 flex flex-col sm:flex-row justify-end gap-3 w-full">
                    <a href="{{ route('admin.tags.index') }}" class="w-full sm:w-auto text-center px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</a>
                    <x-primary-button class="w-full sm:w-auto justify-center">Update Tag</x-primary-button>
                </div>
            </form>
        </div>
    </div>
</x-admin-layout>
