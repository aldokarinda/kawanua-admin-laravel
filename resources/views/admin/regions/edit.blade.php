<x-admin-layout>
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        <x-breadcrumb :items="[['label' => 'Master Data', 'url' => '#'], ['label' => 'Regions', 'url' => route('admin.regions.index')], ['label' => 'Edit', 'url' => '#']]" />

        <div class="mt-4">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Region: {{ $region->name }}</h1>
        </div>

        <div class="mt-6 max-w-3xl glass-panel rounded-xl shadow-sm border border-slate-200 dark:border-slate-700/50 p-6" x-data="{ type: '{{ old('type', $region->type) }}' }">
            <form action="{{ route('admin.regions.update', $region->id) }}" method="POST">
                    ...
                    <div x-show="type === 'city'">
                        ...
                        <select id="parent_id" name="parent_id" class="mt-1 block w-full border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-primary-500 focus:ring-primary-500 rounded-md shadow-sm">
                            ...
                            @foreach($parents as $parent)
                                <option value="{{ $parent->id }}" {{ old('parent_id', $region->parent_id) == $parent->id ? 'selected' : '' }}>{{ $parent->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mt-6 flex flex-col sm:flex-row justify-end gap-3 w-full">
                    <a href="{{ route('admin.regions.index') }}" class="w-full sm:w-auto text-center px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</a>
                    <x-primary-button class="w-full sm:w-auto justify-center">Update Region</x-primary-button>
                </div>
            </form>
        </div>
    </div>
