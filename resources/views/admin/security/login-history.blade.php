<x-admin-layout>
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="sm:flex sm:items-center sm:justify-between mb-8">
            ...
            <div class="mt-4 sm:mt-0 flex gap-2">
                <form method="GET" class="flex gap-2">
                    <select name="status" class="text-sm border border-gray-300 rounded-md dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600">
                        <option value="">All Status</option>
                        <option value="success" {{ request('status') === 'success' ? 'selected' : '' }}>Success</option>
                        <option value="failed" {{ request('status') === 'failed' ? 'selected' : '' }}>Failed</option>
                        <option value="locked" {{ request('status') === 'locked' ? 'selected' : '' }}>Locked</option>
                    </select>
                    <select name="user_id" class="text-sm border border-gray-300 rounded-md dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600">
                        <option value="">All Users</option>
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ request('user_id') == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                        @endforeach
                    </select>
                    <input type="date" name="date_from" value="{{ request('date_from') }}" class="text-sm border border-gray-300 rounded-md dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600">
                    <input type="date" name="date_to" value="{{ request('date_to') }}" class="text-sm border border-gray-300 rounded-md dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600">
                    <button type="submit" class="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-slate-600 text-sm rounded-md bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700"><i class="bi bi-funnel"></i> Filter</button>
                    <a href="{{ route('admin.security.login-history') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-slate-600 text-sm rounded-md bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700">Clear</a>
                </form>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
            ...
            @if($logs->hasPages())
                <div class="px-6 py-4 border-t border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/50">{{ $logs->links() }}</div>
            @endif
        </div>
    </div>
</x-admin-layout>
